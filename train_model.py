...
def test(model, test_loader, criterion, hook):
    ...
    logger.info(
        f"Test set: Average loss: {average_loss}, Average accuracy: {100 * average_accuracy}%"
    )
...
def create_data_loaders(data, batch_size):
    ...
    test_transform = transforms.Compose(
        [transforms.Resize(256), transforms.Resize((224, 224)), transforms.ToTensor()]
    )
    ...
    validation_dataset = torchvision.datasets.ImageFolder(
        root=validation_path, transform=test_transform
    )
...
def main(args):
    ...
    train_data_loader, test_data_loader, validation_data_loader = create_data_loaders(
        data=args.data_dir, batch_size=args.batch_size
    )
    model = train(
        model,
        train_data_loader,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--epochs", type=int, default=2, metavar="N", help="number of epochs to train"
    )
    parser.add_argument(
